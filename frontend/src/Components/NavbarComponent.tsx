...
function NavbarComponent() {
  ...
  return (
    <header className="flex flex-wrap fixed sm:justify-start sm:flex-nowrap z-50 w-full backdrop-blur-xl border-b border-gray-200 text-sm py-3 sm:py-0">
      <nav className="relative max-w-[85rem] w-full mx-auto px-4 sm:flex sm:items-center sm:justify-between sm:px-6 lg:px-8" aria-label="Global">
        <div className="flex items-center justify-between">
          <a className="flex-none text-xl font-semibold" href="/" aria-label="Brand">LearnEra</a>
          <div className="sm:hidden">
            <button type="button" onClick={toggleMenu} className="hs-collapse-toggle size-9 flex justify-center items-center text-sm font-semibold rounded-lg border border-gray-200 text-gray-800 hover:bg-gray-100 disabled:opacity-50 disabled:pointer-events-none" aria-label="Toggle navigation">
        ...
        <div id="navbar-collapse-with-animation" className={isMenuOpen ? "hs-collapse overflow-hidden transition-all duration-300 basis-full grow sm:block" : "hs-collapse hidden overflow-hidden transition-all duration-300 basis-full grow sm:block"}>
          <div className="flex flex-col gap-y-4 gap-x-0 mt-5 sm:flex-row sm:items-center sm:gap-y-0 sm:gap-x-7 sm:mt-0 sm:ps-7">
            <Link to='#home' className="font-medium text-blue-600 sm:py-6" aria-current="page">Home</Link>
            <Link to='#cards' className="font-medium text-gray-500 hover:text-gray-400 sm:py-6">My Cards</Link>
            <Link to='#transactions' className="font-medium text-gray-500 hover:text-gray-400 sm:py-6">Transactions</Link>
            <Link to='#payment-transfer' className="font-medium text-gray-500 hover:text-gray-400 sm:py-6">Payment Transfer</Link>
            <div className="flex items-center gap-x-2 sm:ms-auto">
              <Link className="flex items-center gap-x-2 font-medium text-gray-500 hover:text-blue-600" to='/sign-in'>
