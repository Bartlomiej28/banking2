...
function BuyDepositComponent() {
  ...
  return (
    <div className='w-full min-h-screen h-max bg-black bg-opacity-50 flex justify-center items-center absolute bottom-0 left-0 z-40'>
      <div className='w-11/12 md:w-2/3 lg:w-1/3 h-auto bg-white rounded-xl p-4 flex flex-col gap-3 relative'>
        ...
        <button onClick={handleOpenADeposit} className='px-4 py-2 bg-blue-500 rounded-2xl text-white font-bold mt-4 text-sm md:text-base'>
          {isLoading === true ? <LoadingWindowComponent/> : 'Open a deposit'}
        </button>
      </div>
    </div>
  );
